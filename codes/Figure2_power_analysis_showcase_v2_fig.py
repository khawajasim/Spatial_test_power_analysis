...
def main():
    ...
    if run_sim == 'Y' or run_sim == 'y':   
        ...
        for Nc2 in Nc2_all:
            ...
            outname = '../Data/power_analysis_showcase/powertest-Z%.0f-Ncell%.0f-Nsim%d-sigma%.1f.out' % (Z, 2*Nc2, Nsim,sigma_1)
            f = open(outname, 'w')
    ...
    for ng, grid in enumerate(gridtype):
        ...
        if grid == 'Uniform grid':
            ...
            ax.text(min(xb), max(max(model1), max(nobs)) -0.1, '(a)', fontsize=16)
        if grid == 'Density grid':
            ax.text(min(xb)-0.3, max(max(model0), max(nobs))-0.1, '(b)', fontsize=16)
    ...
    plt.title('$\mathregular{N_{cells}}$ vs Power ($\mathregular{N_{eqs}=%.0f}$, $\sigma=%.1f$)' % (Neqs, sigma_1)) #, fontsize = 14
    plt.xticks([ 2,  4,  8, 16, 32, 64],[2,  4,  8, 16, 32, 64])
    ax.text(2,0.90, '(c)', fontsize=16)
    ax.set_ylim(0,1.02)
    ...
    plt.ylabel('Power') #, fontsize = 14
    plt.title('$\mathregular{N_{eqs}}$ vs Power ($\mathregular{N_{cell}=%.0f}$, $\sigma=%.1f$)' % (Ncell, sigma_1)) #, fontsize = 14
    plt.xticks([ 1,  2,  4,  8, 16, 32, 64],[ 1,  2,  4,  8, 16, 32, 64])
    ax.text(1,0.90, '(d)', fontsize=16)
    ax.set_ylim(0,1.02)
    ...
    for Ncell in Ncell_all:
    #    print('---Number of Cells :', Ncell)
        outname = '../Data/power_analysis_showcase/powertest-Z%.0f-Ncell%.0f-Nsim%d-sigma%.1f.out' % (Z, Ncell, Nsim,sigma_2) #Z=3
        data = np.loadtxt(outname, skiprows=1)
        #The below IF is for the reduced computations we did to reduce computations only to the values we need for figure.
        if data.ndim == 2:
            power_cell = np.row_stack((power_cell,data[data[:,0]==Neqs]))
    ...
    plt.xlabel('$\mathrm{N_{cells}}$') #, fontsize = 14
    plt.ylabel('Power', fontsize = 14)  #, 
    plt.title('$\mathregular{N_{cells}}$ vs Power ($\mathregular{N_{eqs}=%.0f}$, $\sigma=%.1f$)' % (Neqs, sigma_2)) #, fontsize = 14
    plt.xticks([ 2,  4,  8, 16, 32, 64],[2,  4,  8, 16, 32, 64])
    ax.text(2,0.90, '(e)', fontsize=16)
    ax.set_ylim(0,1.02)
    ...
    plt.xlabel('$\mathrm{N_{eqs}}$', fontsize = 16) #, 
    plt.ylabel('Power') #, fontsize = 14
    plt.title('$\mathregular{N_{eqs}}$ vs Power ($\mathregular{N_{cell}=%.0f}$, $\sigma=%.1f$)' % (Ncell,sigma_2)) #, fontsize = 14
    plt.xticks([ 1,  2,  4,  8, 16, 32, 64],[ 1,  2,  4,  8, 16, 32, 64])
    ax.text(1,0.90, '(f)', fontsize=16)
    ax.set_ylim(0,1.02)
    
    
    plt.tight_layout()
    plt.savefig('../Figures/Figure2_stest_power_showcase_sigma_'+str(sigma_1)+'.png', dpi = 400)
    plt.savefig('../Figures/Figure2_stest_power_showcase_sigma_'+str(sigma_1)+'.svg', dpi = 400)
